...
import {
  ...
  type AtomisticPrivatePositionTrajectoryFrameHandleV048,
  type AtomisticPrivatePositionTrajectoryMetadataV048,
} from '../simulation/atomistic-private-position-trajectory-v048.ts';

export const ATOMISTIC_PRIVATE_TRAJECTORY_INSTANCING_PLAN_VERSION_V048 =
  'tf.atomistic-private-trajectory-instancing-plan/0.4.8' as const;
export const ATOMISTIC_PRIVATE_TRAJECTORY_INSTANCING_RUNTIME_VERSION_V048 =
  'tf.atomistic-private-trajectory-instancing-runtime/0.4.8' as const;
export const ATOMISTIC_PRIVATE_TRAJECTORY_INSTANCING_UPDATE_VERSION_V048 =
  'tf.atomistic-private-trajectory-instancing-update/0.4.8' as const;
export const ATOMISTIC_PRIVATE_TRAJECTORY_INSTANCING_SNAPSHOT_VERSION_V048 =
  'tf.atomistic-private-trajectory-instancing-snapshot/0.4.8' as const;
...
export type AtomisticPrivateTrajectoryInstancingPlanV048 = Readonly<{
  schemaVersion: typeof ATOMISTIC_PRIVATE_TRAJECTORY_INSTANCING_PLAN_VERSION_V048;
  role: 'private-discrete-position-trajectory-gpu-instancing-plan-no-solver';
  sourceBinding: Readonly<{
    sourceVersion: 'tf.atomistic-private-position-trajectory/0.4.8';
    ...
    allowedStateKeys: ReadonlyArray<string>;
  ...
  atomBatches: readonly [
    Readonly<{
      batchId: 'atom:O'; element: 'O'; instanceCount: 895;
      atomIndicesByInstance: ReadonlyArray<number>; displayRadiusNanometer: 0.065;
    }>,
    Readonly<{
      batchId: 'atom:H'; element: 'H'; instanceCount: 1_790;
      atomIndicesByInstance: ReadonlyArray<number>; displayRadiusNanometer: 0.04;
    }>,
  ];
  ...
  scientificBoundary: Readonly<{
    renderedChannel: 'digest-bound-discrete-source-position-f32-derivative-only';
...
export type AtomisticPrivateTrajectoryInstancingUpdateV048 = Readonly<{
  schemaVersion: typeof ATOMISTIC_PRIVATE_TRAJECTORY_INSTANCING_UPDATE_VERSION_V048;
  status: 'private-discrete-position-frame-projected-atomically';
...
export type AtomisticPrivateTrajectoryInstanceSelectionV048 = Readonly<{
...
export function createAtomisticPrivateTrajectoryInstancingPlanV048(
  metadataInput: AtomisticPrivatePositionTrajectoryMetadataV048,
): AtomisticPrivateTrajectoryInstancingPlanV048 {
  const metadata = assertAtomisticPrivatePositionTrajectoryMetadataV048(metadataInput);
  ...
  const plan = Object.freeze({
    schemaVersion: ATOMISTIC_PRIVATE_TRAJECTORY_INSTANCING_PLAN_VERSION_V048,
    role: 'private-discrete-position-trajectory-gpu-instancing-plan-no-solver' as const,
    sourceBinding: Object.freeze({
      sourceVersion: 'tf.atomistic-private-position-trajectory/0.4.8' as const,
      ...
      orderedFrameDigest: metadata.binding.orderedFrameDigest,
      sourcePositionsArtifactDigest: metadata.binding.sourcePositionsArtifactDigest,
      ...
      allowedStateKeys: Object.freeze(metadata.sequence.frames.map((frame) => frame.stateKey)),
    ...
    atomBatches: Object.freeze([
      Object.freeze({ batchId: 'atom:O' as const, element: 'O' as const,
        instanceCount: 895 as const, atomIndicesByInstance: oxygenIndices,
        displayRadiusNanometer: 0.065 as const }),
      Object.freeze({ batchId: 'atom:H' as const, element: 'H' as const,
        instanceCount: 1_790 as const, atomIndicesByInstance: hydrogenIndices,
    ...
    scientificBoundary: Object.freeze({
      renderedChannel: 'digest-bound-discrete-source-position-f32-derivative-only' as const,
  ...
  PLAN_DATA.set(plan, Object.freeze({ metadata, atoms: topology.atoms, links: topology.links }));
  return plan;
}

export class AtomisticPrivateTrajectoryInstancingRuntimeV048 {
  readonly schemaVersion = ATOMISTIC_PRIVATE_TRAJECTORY_INSTANCING_RUNTIME_VERSION_V048;
  readonly plan: AtomisticPrivateTrajectoryInstancingPlanV048;
  ...
  constructor(plan: AtomisticPrivateTrajectoryInstancingPlanV048) {
    const data = PLAN_DATA.get(plan);
    if (!data) throw new Error('private trajectory instancing plan lacks factory identity');
    this.plan = plan;
    this.#data = data;
  }

  updatePrivatePositionFrameV048(
    handle: AtomisticPrivatePositionTrajectoryFrameHandleV048,
  ): AtomisticPrivateTrajectoryInstancingUpdateV048 {
    this.#assertActive();
    if (!handle || typeof handle !== 'object'
      || typeof handle.copyPositionBytes !== 'function'
      || typeof handle.isRevoked !== 'function'
      || handle.isRevoked()) {
      throw new Error('private trajectory instancing requires one active frame handle');
    }
    const frame = handle.frame;
    const expected = this.#data.metadata.sequence.frames[frame?.frameOrdinal];
    if (!expected || handle.trajectoryMetadataDigest !== this.plan.sourceBinding.trajectoryMetadataDigest
      ...
      || handle.binding.orderedFrameDigest !== this.plan.sourceBinding.orderedFrameDigest
      || handle.binding.sourcePositionsArtifactDigest
        !== this.plan.sourceBinding.sourcePositionsArtifactDigest
      ...
      || frame.stateKey !== expected.stateKey
      || frame.sourceFrameDigest !== expected.sourceFrameDigest
      || frame.derivedPositionsF32Digest !== expected.derivedPositionsF32Digest) {
      throw new Error('private trajectory frame is not bound to this instancing plan');
    }
    ...
    try {
      ...
      if (digestBytes(bytes) !== expected.derivedPositionsF32Digest) {
        throw new Error('private trajectory frame bytes changed');
      }
    ...
    this.#stateKey = expected.stateKey;
    this.#positionsDigest = expected.derivedPositionsF32Digest;
    return Object.freeze({
      schemaVersion: ATOMISTIC_PRIVATE_TRAJECTORY_INSTANCING_UPDATE_VERSION_V048,
      status: 'private-discrete-position-frame-projected-atomically' as const,
      sourceFrameOrdinal: expected.frameOrdinal,
      sourceFrameDigest: expected.sourceFrameDigest,
      stateKey: expected.stateKey,
      positionsDerivedF32Digest: expected.derivedPositionsF32Digest,
      ...
      forceOrVelocityConsumed: false as const,
      physicalWorldState: false as const,
      publicDistributionEligible: false as const,
    });
  }
  ...
  snapshot() {
    this.#assertActive();
    return Object.freeze({
      schemaVersion: ATOMISTIC_PRIVATE_TRAJECTORY_INSTANCING_SNAPSHOT_VERSION_V048,
      sourceFrameOrdinal: this.#sourceFrameOrdinal,
      sourceFrameDigest: this.#sourceFrameDigest,
      presentationFrameDigest: this.#stateKey,
      positionsDerivedF32Digest: this.#positionsDigest,
      atomOrderDigest: this.plan.sourceBinding.atomOrderDigest,
      topologyDigest: this.plan.sourceBinding.topologyDigest,
      publicDistributionEligible: false as const,
      atomMatricesByBatch: Object.freeze([
        Object.freeze({ batchId: 'atom:O' as const, matrices: this.#current.oxygenMatrices.slice() }),
        Object.freeze({ batchId: 'atom:H' as const, matrices: this.#current.hydrogenMatrices.slice() }),
      ] as const),
  ...
  resolveAtomInstanceSelection(batchId: 'atom:O' | 'atom:H', instanceId: number):
  AtomisticPrivateTrajectoryInstanceSelectionV048 {
    this.#assertActive();
    ...
    if (!batch || !Number.isSafeInteger(instanceId) || instanceId < 0
      ...
      throw new Error('private trajectory atom selection is outside its batch');
    }
  ...
  #assertActive() {
    if (this.#disposed) throw new Error('private trajectory instancing runtime is disposed');
  }
}
...
function copyFixedPositionBytes(value: Uint8Array) {
  if (!(value instanceof Uint8Array)
    ...
    || value.byteLength !== FRAME_BYTE_LENGTH) {
    throw new Error('private trajectory positions require one intrinsic 32,220-byte Uint8Array');
  }
  let buffer: unknown;
  let resizable = false;
  try {
    ...
    throw new Error('private trajectory position bytes lack stable ArrayBuffer ownership');
  }
  if (!(buffer instanceof ArrayBuffer)
    || Object.getPrototypeOf(buffer) !== ArrayBuffer.prototype
    || resizable) {
    throw new Error('private trajectory position bytes reject shared or resizable buffers');
  }
  ...
  try {
    for (let index = 0; index < positions.length; index += 1) {
      ...
      if (!Number.isFinite(value) || Object.is(value, -0)) {
        throw new Error(`private trajectory position component ${index} is invalid`);
      }
...
function projectInto(data: RuntimePlanData, positions: Float32Array, target: RuntimeBuffers) {
  for (const atom of data.atoms) {
    ...
    target.displayPositions[offset] = point[0];
    target.displayPositions[offset + 1] = point[1];
    target.displayPositions[offset + 2] = point[2];
    const matrices = atom.element === 'O' ? target.oxygenMatrices : target.hydrogenMatrices;
    const instance = atom.element === 'O' ? atom.atomIndex / 3 : atom.atomIndex - Math.floor(atom.atomIndex / 3) - 1;
    const normalizedInstance = atom.element === 'O'
      ? Math.floor(instance)
      : Math.floor(atom.atomIndex / 3) * 2 + (atom.atomIndex % 3) - 1;
    writeAtomMatrix(matrices, normalizedInstance * 16,
      atom.element === 'O' ? 0.065 : 0.04, point[0], point[1], point[2]);
  }
  for (const link of data.links) {
    const a = link.atomAIndex * 3;
    const b = link.atomBIndex * 3;
    writeLinkMatrix(target.topologyLinkMatrices, link.topologyLinkIndex * 16,
      target.displayPositions[a], target.displayPositions[a + 1], target.displayPositions[a + 2],
      target.displayPositions[b], target.displayPositions[b + 1], target.displayPositions[b + 2]);
  }
}
...
function writeAtomMatrix(
  target: Float32Array, offset: number, scale: number, x: number, y: number, z: number,
) {
  target[offset] = scale; target[offset + 1] = 0; target[offset + 2] = 0; target[offset + 3] = 0;
  target[offset + 4] = 0; target[offset + 5] = scale; target[offset + 6] = 0; target[offset + 7] = 0;
  target[offset + 8] = 0; target[offset + 9] = 0; target[offset + 10] = scale; target[offset + 11] = 0;
...
function writeLinkMatrix(
  ...
  if (!Number.isFinite(length) || length <= Number.EPSILON) {
    throw new Error('private trajectory topology link has invalid displayed length');
  }
  ...
  const radius = 0.012;
  target[offset] = xx * radius; target[offset + 1] = xy * radius; target[offset + 2] = xz * radius; target[offset + 3] = 0;
  target[offset + 4] = ux * length; target[offset + 5] = uy * length; target[offset + 6] = uz * length; target[offset + 7] = 0;
  target[offset + 8] = zx * radius; target[offset + 9] = zy * radius; target[offset + 10] = zz * radius; target[offset + 11] = 0;
  target[offset + 12] = (ax + bx) / 2; target[offset + 13] = (ay + by) / 2; target[offset + 14] = (az + bz) / 2; target[offset + 15] = 1;
}
  ...
  for (const values of [
    ...
    for (const value of values) {
      if (!Number.isFinite(value)) {
        throw new Error('private trajectory projection is not finite float32');
      }
    }
  }
}
  ...
  for (let water = 0; water < WATER_COUNT; water += 1) {
    ...
    atoms.push(
      Object.freeze({ atomIndex: oxygen, atomId: `${moleculeId}:O`, moleculeId,
        element: 'O' as const, anchorOxygenIndex: oxygen }),
      Object.freeze({ atomIndex: oxygen + 1, atomId: `${moleculeId}:H1`, moleculeId,
        element: 'H' as const, anchorOxygenIndex: oxygen }),
      Object.freeze({ atomIndex: oxygen + 2, atomId: `${moleculeId}:H2`, moleculeId,
        element: 'H' as const, anchorOxygenIndex: oxygen }),
    );
    links.push(
      Object.freeze({ topologyLinkIndex: water * 2, atomAIndex: oxygen, atomBIndex: oxygen + 1 }),
      Object.freeze({ topologyLinkIndex: water * 2 + 1, atomAIndex: oxygen, atomBIndex: oxygen + 2 }),
    );
  }
