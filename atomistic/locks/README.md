...
This candidate checks in exact, hash-locked Python 3.12/Linux x86_64 runtime
closures for MatterSim and MACE. The locks and their canonical runtime-input
manifests are byte-identical across accepted protected-main runs `33242996794`
and `33242999376`; Commit V independently authenticated both source runs and
all four artifacts, and Commit F froze the stable identities at **2 / 2**. They are candidate
runtime inputs only—not evidence of a 693-record run, accuracy, reproducibility,
an OCI image identity, binary-redistribution permission or promotion.
...
The `*.bootstrap.in` files remain reviewable resolution intent, not hash locks.
The `*.requirements.lock` files are the accepted exact closures generated by
the following completed bootstrap sequence; that sequence still must not be
reported as a full scientific reproduction:
...
The checked-in locks are bound by
`evaluation/atomistic/full-candidate-execution-preflight.json` to their exact
sizes and SHA-256 values. The same preflight keeps the executable producer
identity unconfigured, every dispatch gate false and publication disabled.
Neither a lock file nor a Dockerfile authorizes an image build or a benchmark
dispatch; those require a separately reviewed vNext execution contract.
...
`promotionEligible: false`, and the enclosing runs remain
`bootstrap-not-reproduced`. The two accepted protected-main replicas agree on
the derived wheel digest, member inventory and canonical MACE runtime-input
manifest. That agreement permits the private runtime-input freeze; it does not
grant binary redistribution. `python-hostlist` 2.3.0 is GPL-2.0-or-later, so a
public container or wheelhouse remains blocked until the applicable source,
notice and distribution obligations are recorded (or the dependency is
replaced by an independently reviewed upstream fix).
...
as negative protocol evidence and cannot be rewritten or counted later. Those
R6b runs contributed **0** accepted replicas, so the state at that stage was
**0 / 2**; later S→V→F observations established the current stable-input freeze
at **2 / 2**. Exact R6b jobs, artifacts and archive digests are recorded in
`evaluation/reviews/2026-08-29-r6b-successful-execution-claim-conflict-review.md`.

The non-circular P→S→V→F sequence is complete for stable bootstrap input
identity only. P quarantined the legacy path and prepositioned v2; S selected
P's immutable source and produced two fresh protected-main observations; V
authenticated their original bytes without rewriting claims; F froze the two
accepted identities. The present execution preflight still leaves the producer
disabled. A publication postprocessor may reject contradictory output but must
never sanitize it into acceptable evidence.
