#!/usr/bin/env python3
"""Run a frozen atomistic checkpoint without evaluating scientific accuracy.
...
RUNNER_SCHEMA = "tf.atomistic-run-summary/0.2"
PREDICTION_SCHEMA = "tf.atomistic-prediction/0.2"
DIAGNOSTIC_SCHEMA = "tf.atomistic-run-diagnostic/0.2"
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run MatterSim or MACE under the frozen v0.2 "
...
def _runner_identity() -> dict[str, Any]:
    paths = [Path(__file__).resolve(), Path(__file__).with_name("runtime_contract.py").resolve()]
    files = sorted(({"name": path.name, "sha256": f"sha256:{sha256_file(path)}"}
                    for path in paths), key=lambda item: item["name"])
...
def _validate_provenance(runner_plan: dict[str, Any], model_name: str,
    ...
    actual_container = provenance["containerImageDigest"]
    if expected_container is not None and expected_container != actual_container:
        raise ContractViolation("container-digest-mismatch", "container digest differs",
                                details={"expected": expected_container,
...
def _write_failure(output: Path, *, run_id: str, stage: str,
    ...
    for name in ("predictions.jsonl", "run-diagnostics.json", "summary.json",
        ...
        try:
            ...
            pass
    atomic_write_json(output / "diagnostics.json", {
...
def run(args: argparse.Namespace, output: Path, run_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        environment_binding = {
            ...
            "containerImageDigest": provenance["containerImageDigest"],
        ...
        summary = {
            ...
            "environment": {
                **environment_binding, "environmentSha256": environment_sha256,
                ...
                "structureBundleSha256": artifacts["structures"].sha256,
                "structureManifestFileSha256": artifacts["structureManifest"].sha256,
                "planPath": str(plan_path), "sourceRevision": os.environ.get("GITHUB_SHA"),
            },
            "counts": {
