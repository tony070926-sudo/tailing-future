...
DOCKER_CONFIG_IMAGE_ID_RE = re.compile(r"^sha256:[0-9a-f]{64}$")
GIT_REVISION_RE = re.compile(r"^[0-9a-f]{40}$")
PROMOTION_CLAIM_KEYS = frozenset(
    ("promotionEligible", "promotionTrustRoot", "comparable", "reproduced")
)
EVIDENCE_CLASS = "bootstrap-not-reproduced"
WORKFLOW_REVISION_ENV = "TAILING_ATOMISTIC_WORKFLOW_REVISION"
RUNTIME_SOURCE_REVISION_ENV = "TAILING_ATOMISTIC_RUNTIME_SOURCE_REVISION"
DOCKER_CONFIG_IMAGE_ID_ENV = "TAILING_ATOMISTIC_DOCKER_LOCAL_CONFIG_IMAGE_ID"
THREAD_ENVIRONMENT = {
    "OMP_NUM_THREADS": "1",
    "MKL_NUM_THREADS": "1",
...
def stable_file_record(
    ...
    *,
    ...
    """Hash one bounded, single-link file and reject path or fd identity drift."""

    if max_bytes <= 0 or chunk_bytes <= 0:
        raise ValueError("file bounds must be positive")
    canonical = _canonical_existing_file(str(path), label)
    path_before = os.lstat(canonical)
    if path_before.st_nlink != 1:
        raise ContractViolation(
            "file-links-invalid", f"{label} must be one single-link regular file"
        )
    ...
    descriptor = _open_readonly_nofollow(canonical)
    try:
        descriptor_before = os.fstat(descriptor)
        if identity(descriptor_before) != identity(path_before):
            raise ContractViolation(
                "file-identity-drift", f"{label} changed before it could be read"
            )
        if descriptor_before.st_size > max_bytes:
            raise ContractViolation(
                "file-too-large", f"{label} exceeds the {max_bytes}-byte bound"
            )
        digest = hashlib.sha256()
        total = 0
        while True:
        ...
        if total != descriptor_before.st_size or identity(descriptor_after) != identity(
            descriptor_before
        ):
            raise ContractViolation(
                "file-identity-drift", f"{label} changed while it was read"
            )
    finally:
        os.close(descriptor)

    try:
        path_after = os.lstat(canonical)
        resolved_after = canonical.resolve(strict=True)
    except OSError as error:
        raise ContractViolation(
            "file-identity-drift", f"{label} disappeared after it was read"
        ) from error
    if resolved_after != canonical or identity(path_after) != identity(descriptor_after):
        raise ContractViolation(
            "file-identity-drift", f"{label} path changed after it was read"
        )
    return {
        "sizeBytes": descriptor_after.st_size,
...
def non_promotional_claims() -> dict[str, Any]:
    """Return the only claim boundary emitted by the v2 bootstrap runner."""
...
def assert_no_positive_promotion_claims(value: Any, path: str = "$") -> None:
    """Recursively reject any positive or malformed promotion claim."""

    if isinstance(value, Mapping):
        for key, child in value.items():
            child_path = f"{path}.{key}"
            if key in PROMOTION_CLAIM_KEYS and child is not False:
                raise ContractViolation(
                    "promotion-claim-positive",
                    f"{child_path} must be exactly false in bootstrap evidence",
                )
            assert_no_positive_promotion_claims(child, child_path)
    elif isinstance(value, (list, tuple)):
        for index, child in enumerate(value):
            assert_no_positive_promotion_claims(child, f"{path}[{index}]")


def require_execution_revisions(
    environ: Mapping[str, str] | None = None,
) -> tuple[str, str]:
    """Require separate, unambiguous bindings for workflow and runtime source."""

    source = os.environ if environ is None else environ
    revisions: list[str] = []
    for variable in (WORKFLOW_REVISION_ENV, RUNTIME_SOURCE_REVISION_ENV):
        revision = source.get(variable)
        if not isinstance(revision, str) or not GIT_REVISION_RE.fullmatch(revision):
            raise ContractViolation(
                "revision-invalid",
                f"{variable} must be a full lowercase 40-hex Git revision",
            )
...
def execution_provenance(
    ...
    """Capture execution identity without converting completeness into promotion."""

    source = os.environ if environ is None else environ
    workflow_revision, runtime_source_revision = require_execution_revisions(source)
    lock_path_raw = source.get("TAILING_ATOMISTIC_LOCK_PATH")
    lock_record: dict[str, Any] | None = None
    if lock_path_raw:
        lock_path = _canonical_existing_file(lock_path_raw, "environment lock")
        lock_identity = stable_file_record(
            lock_path, "environment lock", max_bytes=16 * 1024 * 1024
        )
        lock_record = {
            "path": str(lock_path),
            **lock_identity,
        }
    config_image_id = source.get(DOCKER_CONFIG_IMAGE_ID_ENV)
    if config_image_id is not None and (
        not isinstance(config_image_id, str)
        or not DOCKER_CONFIG_IMAGE_ID_RE.fullmatch(config_image_id)
    ):
        raise ContractViolation(
            "docker-config-image-id-invalid",
            f"{DOCKER_CONFIG_IMAGE_ID_ENV} must be sha256:<64 lowercase hex>",
        )
    execution_identity_complete = lock_record is not None and config_image_id is not None
    provenance = {
        "environmentLock": lock_record,
        "containerIdentity": {
            "kind": "docker-local-load-config-id",
            "configImageId": config_image_id,
            "scope": "run-specific",
        ...
        "executionIdentityComplete": execution_identity_complete,
        **non_promotional_claims(),
        "note": (
            "dependency lock and Docker local-load config image ID recorded; "
            "the config ID is run-specific and is not a registry manifest digest "
            "or promotion trust root"
            if execution_identity_complete
            else "bootstrap evidence only: dependency lock and/or Docker local-load "
            "config image ID is absent; no promotion claim"
        ),
    }
    assert_no_positive_promotion_claims(provenance)
    return provenance
