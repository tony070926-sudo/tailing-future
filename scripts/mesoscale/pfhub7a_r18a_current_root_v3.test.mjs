...
import { persistExecutionEvidence, requireCurrentAdmission, requireUnchangedCandidate } from './pfhub7a_r18a_current_root_v3.mjs';
import { createExecutionBudget, NUMERICAL_BUDGET_MS, STOP_GRACE_MS } from './pfhub7a_r18a_current_root.mjs';
import { INTEGRATION_BASE, SCIENTIFIC_CONTRACT_BASE } from '../backend-migration/verify-r18a-wrangler-admission-v0.4.mjs';
...
describe('versioned current-tree runner contract, without numerical execution', () => {
  it('retains machine-readable abstention when execution evidence cannot be persisted', () => {
    const temporary = realpathSync(mkdtempSync(path.join(tmpdir(), 'tf-v04-evidence-contract-')));
    try {
  ...
  it('abstains with unknown integration identity when caller inputs are missing', () => {
    const result = spawnSync(process.execPath, ['scripts/mesoscale/pfhub7a_r18a_current_root_v3.mjs'], { encoding: 'utf8', timeout: 30000 });
    expect(result.status).toBe(1);
    const evidence = JSON.parse(result.stdout);
    expect(evidence).toMatchObject({ schemaVersion: 'tf.r18a-current-root-execution/0.4', scientificContractBaseCommit: SCIENTIFIC_CONTRACT_BASE, integrationBaseCommit: null, candidateTreeIdentity: null, sourceInputIdentity: null, status: 'abstain', releaseEligible: false, scientificPromotionEligible: false, commands: [] });
  ...
  it('tests the exact production final block: late admission and candidate drift cannot mark success', () => {
    const source = readFileSync(new URL('./pfhub7a_r18a_current_root_v3.mjs', import.meta.url), 'utf8');
